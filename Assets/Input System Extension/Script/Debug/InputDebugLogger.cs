...
{
    [AddComponentMenu("Tools/Input System Extension/Debug/Input Debug Logger")]
        ...
        private static readonly Dictionary<KeyCode, string> ManualMap = new()
    {
            ...
            foreach (KeyCode kc in Enum.GetValues(typeof(KeyCode)))
            {
                try
                {
                    ...
                    if (data != null && data.KeyCodes != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(newPath)) indexByName = data.KeyCodes.FindIndex(k => string.Equals(k.keyName, newPath, StringComparison.OrdinalIgnoreCase));
                        ...
                        if (indexByName >= 0) hasSpriteByName = data.KeyCodes[indexByName].sprite != null;
                    }
        ...
        private string ResolveNewPathForKey(KeyCode kc, List<string> pressedNewPaths)
        {
            ...
            foreach (var p in pressedNewPaths)
            {
                ...
                if (candidate.Contains(normK) || normK.Contains(candidate)) return p;
            }
