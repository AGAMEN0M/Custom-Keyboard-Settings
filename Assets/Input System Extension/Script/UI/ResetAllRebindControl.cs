...
namespace InputSystemExtension
{
    ...
    /// </summary>
    [AddComponentMenu("Tools/Input System Extension/UI/Rebind/Reset All Rebind Control")]
    public class ResetAllRebindControl : MonoBehaviour
    {
        ...
        private void ResetAll()
        {
            ...
            {
                if (manager != null) manager.ResetToDefault(); // Reset this manager to its default binding.
            }

            // Iterate through each RebindControlManagerTMP and reset to default bindings.
            foreach (var managerTMP in rebindControlsTMP)
            {
                if (managerTMP != null) managerTMP.ResetToDefault(); // Reset this TMP-based manager to its default binding.
            }
        }
